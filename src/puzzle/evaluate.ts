...
export function _evaluate(problem: Problem, actions: Action[]): StateTransition {
...
export function evaluateNoMem(problem: Problem, actions: Action[]): StateTransition {
    if (actions.length === 0) { return { state: initialState(problem) }; }
    const { state: prevState } = evaluateNoMem(problem, actions.slice(0, -1));
    const action = actions[actions.length - 1];
    return actRound(action, prevState);
}